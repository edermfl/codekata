...
import br.com.ederleite.codekata.domino.service.impl.JogoDominoServiceImplPimenta
...
public class JogoDominoServiceTest {
    ...
    private final IJogoDominoService service = new JogoDominoServiceImplPimenta();
	...
	// os erro serÃ£o adicionados nessa lista
	...
	for (String linhaTxt : linhasTestTable) {
		...
		tabuleiro.getPecasEncaixadas().eachWithIndex { PecaDomino pecaAtual, i ->
		    // ponta A da prÃ³xima peÃ§a Ã© igual a ponta B da peÃ§a anterior
		    if (pecaAnterior.pontaB == null || pecaAtual.pontaA == pecaAnterior.pontaB) {
			pecaAnterior = pecaAtual
		    } else {
			erros.add("Cenario $cenario falhou: Na ${i}Âº peÃ§a encaixada, a pontaB ${pecaAnterior.pontaB} nÃ£o se encaixa na pontaA ${pecaAtual.pontaA} da prÃ³xima peÃ§a. Segue jogo: ${tabuleiro.getPecasEncaixadas()}");
			System.out.println(mensagemFalha + tabuleiro);
		    }
		}
	    ...
	    System.out.println("CenÃ¡rio " + cenario + ": ok! " + (tabuleiro != null ? "Saida:\n" + tabuleiro : ""));
	}

	// se lista de erros contiver algum item, entÃ£o falho o teste e exibo os erros.
	if (!erros.isEmpty()) {
